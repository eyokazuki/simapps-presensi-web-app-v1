...
class User_kla extends CI_Controller
{
	private $grant;
	private $ses;
	...
	{
		parent::__construct();
		$this->load->model("user_kla_m", "dbmodels");
		$this->auth->authlog();
	...
	public function index()
	{
		...
		$getPriv	 					= $this->dbmodels->detail_priv(["aktif" => 1, "id_priv !=" => 1]);
		...
		$this->load->view('user_kla_v', $data);
	}
	...
	public function tabel_user()
	{
		...
		$columns = array(
			...
			array(
				...
				'field' 	=> 'aktif',
				'formatter' => function ($d, $row) {
					return $d == 1 ? "<span class='badge bg-primary'>Aktif</span>" : "<span class='badge bg-secondary'>Tidak Aktif</span>";
				}
			),
			array(
				'db'        => 'a.id_user',
				...
				'formatter' => function ($d, $row) {
					...
					$button 	= "";
					if ($this->grant["edit"]) {
						$button 	.= "<i class='bi bi-pencil-square text-primary btn-icon' onclick='detail_user(\"" . $id . "\")' title='Edit User'></i> ";
					}

					if ($this->grant["delete"]) {
						$button 	.= " &nbsp;&nbsp;<i class='bi bi-x-circle-fill text-danger btn-icon' onclick='hapus(\"" . $id . "\")' title='Hapus User'></i> ";
					}
