...
class Reportbulanan_m extends CI_Model
{
    ...
    public function get_absen_bulanan($startDate, $endDate, $idDepartement, $isAdmin)
    {
        ...
        SELECT
            ...
            (SELECT COUNT(me.id_employee) FROM `m_employee` me WHERE me.`aktif` = 1 AND me.`id_departemen` = $idDepartement) - COUNT(da.`id_employee`) tidak_hadir
        FROM `detail_absensi` da
    ...
    public function get_count_employee($idDepartement, $isAdmin)
    {
        $query = "SELECT COUNT(me.id_employee) jumlah FROM m_employee me WHERE me.aktif = 1";
        if (!$isAdmin) {
            $query .= " AND me.id_departemen = $idDepartement";
        }
