					...
					<div class="swiper-slide">
						<div class="row">
							<div class="col-12">
								...
								<small>Halaman List section</small>
							</div>
						</div>
					</div>
				</div>
		...
		<div class="col-12 col-xl-12 col-sm-12 order-1 order-lg-2 mb-4 mb-lg-0">
			<div class="card">
				<div class="mt-4">
					...
					?>
						<h5 class="card-title">
							<button class="btn btn-primary btn-sm" onclick="tambah_section()" data-bs-toggle='tooltip' data-bs-placement='top' data-bs-original-title='Tambah section'>
								<i class="ti ti-pencil-plus"></i> &nbsp;Section&nbsp;&nbsp;
							</button>
						</h5>
					<?php
					}
				...
				<div class="card-datatable table-responsive mt-4">
					...
					<table id="tabel_section" class="dt-responsive table table-striped table-hover" width="100%" cellspacing="0">
						<thead>
							<tr>
					...
					<div class="row">
						<div class="col-lg-5">
							<div class="form-group">
								<label>Section</label>
								<input type="hidden" class="form-control add-section" id="tipe" name="tipe" required>
								<input type="hidden" class="form-control add-section" id="id" name="id" required>
								<input type="text" class="form-control add-section" id="section" name="section" placeholder="Section" maxlength="100" required>
								<small class="form-text text-muted">
									Section harus diisi dan maksimal 100 karakter.
								</small>
...
<script>
	...
	function hapus(id) {
		swal({
				title: "Hapus section?",
				text: "Yakin ingin hapus Section ini ?",
				icon: "warning",
				buttons: true,
				dangerMode: true,
			})
				...
				if (isConfirm) {
					...
					$.ajax({
						...
						error: function(response) {
							invalid_token(response);
							loadingHide();
							swal("Gagal !", "Section gagal dihapus", "error");
						}
					});
				}
			});
	...
	function detail_section(id) {
		$(".add-section").val('');
		loadingShow();
		$.ajax({
			url: "section/detail_section",
			type: "POST",
			data: {
				id: id,
			},
			...
			success: function(data, status, xhr) {
				...
				$("#code").val(data.code);
				$("#header_title").html("Form Update Section");
				$("#tipe").val("2");
				$("#id").val(id);
				if (data.aktif == 1) {
	...
	function tambah_section() {
		...
		$("#aktif").prop("checked", true);
		$("#tipe").val("1");
		$("#header_title").html("Form Tambah Section");
		$("#ModalForm").modal("show");
	}
		...
		$.ajax({
			...
			error: function(response) {
				...
				if ($("#tipe").val() == 1) {
					...
					swal("Gagal", "Section gagal diupdate", "error");
				}
			}
		});
	});
