		...
		<div class="col-12 col-xl-12 col-sm-12 order-1 order-lg-2 mb-4 mb-lg-0">
			<div class="card">
				<div class="mt-4">
					...
					?>
						<h5 class="card-title">
							<button class="btn btn-primary btn-sm" onclick="tambah_jabatan()" data-bs-toggle='tooltip' data-bs-placement='top' data-bs-original-title='Tambah jabatan'>
				...
				<div class="card-datatable table-responsive mt-4">
					...
					<table id="tabel_jabatan" class="dt-responsive table table-striped table-hover" width="100%" cellspacing="0">
						<thead>
							<tr>
					...
					<div class="row">
						<div class="col-lg-5">
							<div class="form-group">
								...
								<input type="hidden" class="form-control add-jabatan" id="tipe" name="tipe" required>
								<input type="hidden" class="form-control add-jabatan" id="id" name="id" required>
								<input type="text" class="form-control add-jabatan" id="jabatan" name="jabatan" placeholder="jabatan" maxlength="100" required>
								<small class="form-text text-muted">
									jabatan harus diisi dan maksimal 100 karakter.
								</small>
...
<script>
				...
				if (isConfirm) {
					...
					$.ajax({
						...
						error: function(response) {
							invalid_token(response);
							loadingHide();
							swal("Gagal !", "jabatan gagal dihapus", "error");
						}
					});
				}
			});
	...
	function detail_jabatan(id) {
		...
		$.ajax({
			...
			success: function(data, status, xhr) {
				...
				$("#code").val(data.code);
				$("#header_title").html("Form Update jabatan");
				$("#tipe").val("2");
				$("#id").val(id);
				if (data.aktif == 1) {
	...
	function tambah_jabatan() {
		...
		$("#aktif").prop("checked", true);
		$("#tipe").val("1");
		$("#header_title").html("Form Tambah jabatan");
		$("#ModalForm").modal("show");
	}
	...
	$("#form_modal").submit(function(e) {
		...
		$.ajax({
			...
			error: function(response) {
				...
				loadingHide();
				if ($("#tipe").val() == 1) {
					swal("Gagal", "jabatan gagal ditambahkan", "error");
				} else {
					swal("Gagal", "jabatan gagal diupdate", "error");
				}
			}
		});
	});
