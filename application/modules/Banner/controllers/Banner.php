...
class Banner extends CI_Controller
{
	...
	{
		parent::__construct();
		$this->load->model("banner_m", "dbmodels");
		$this->auth->authlog();
	...
	public function index()
	{
		...
		$noUrut = $this->dbmodels->getMaxNoUrut();
		if ($noUrut){
			$noUrut = $noUrut->no_urut+1;
		} else {
			$noUrut = 1;
		}
		$data["maxNoUrut"] = $noUrut;
		...
		$columns = array(
			...
			array(
				...
				'formatter' => function ($d, $row) {
					...
					src='".base_url("assets/images/banner/".$d."")."'>";
					return $gambar;
				}
			),
			array(
			...
			array(
				...
				'formatter' => function ($d, $row) {
					...
					if ($this->grant["edit"]) {
						//$button 	.= "<li><a class='dropdown-item' href='#' onclick='detail_banner(\"" . $id . "\")'><i class='ti ti-pencil'></i> Edit banner</a></li>";
						$button 	.= "<li><a class='dropdown-item' href='#' onclick='detail_banner_no_urut(\"" . $id . "\")'><i class='ti ti-pencil'></i>  Update No Urut</a></li>";
					}

					if ($this->grant["delete"]) {
						$button 	.= "<li><a class='dropdown-item' href='#' onclick='hapus(\"" . $id . "\")'><i class='ti ti-trash'></i>  Hapus banner</a></li>";
					}
					$button .=        "
                                    </ul>
                                </div>";
	...
	public function hapus_banner()
	{
		...
		$log = $this->dbmodels->detail_banner($params_where)->row();
		$db 		= $this->dbmodels->hapus_banner($params_where);

		if ($db == true) {
			$this->auth->simpan_log("m_banner", "delete", $this->ses["s_id_user"], json_encode($log));
	...
	public function detail_banner()
	{
		...
		$detail = $this->dbmodels->detail_banner($params);
		$obj 		= array();
	...
	public function update_aktif()
	{
		...
		$simpan = $this->dbmodels->update_banner($params, $params_where);		
	...
	public function simpan_banner()
	{
		...
		$noUrut 		= xssInput($this->input->post("no_urut"));
		...
		if ($tipe == 1) {
			...
			if ($tipe == 1) {
				if($this->upload->do_upload("file_location")){
					...
					$simpan_db 	= $this->dbmodels->simpan_banner($params);
				...
				$log = $this->dbmodels->detail_banner($params_where)->row();
				$simpan_db 	= $this->dbmodels->update_banner($params, $params_where);
				$proses_msg 	= "diubah";
				$tipe_log = "update";
			}

			if ($simpan_db == true) {
				$this->auth->simpan_log("m_banner", $tipe_log, $this->ses["s_id_user"], json_encode($log));

				$success 	= 1;
				$message	= "banner berhasil $proses_msg";
				$header 	= "Berhasil";
				$tipenotif 	= "success";
			} else {
				$message	= "banner gagal $proses_msg";
			}
	...
	public function update_no_urut()
	{
		ini_set('max_execution_time', '0');
		$id 		= $this->encryption->decrypt($this->input->post("id"));
		$noUrut 		= xssInput($this->input->post("no_urut"));
	...
	public function check_id()
	{
		...
		$count = $this->dbmodels->detail_banner(["id_banner" => $id])->num_rows();
		if ($count > 0) {
			return true;
		} else {
			$this->form_validation->set_message("check_id", "Data banner invalid");
			return false;
		}
	}
}
